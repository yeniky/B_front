...
import ZoneHistoryFilterModal from 'components/MetricsView/CsvModals/ZoneHistory/ZoneHistoryFiltersModal.container';
...
const ZoneHistory = () => {
  ...
  const pageInfo = usePagination(tagService.getZoneMetrics, 1, 3);
  ...
  const handleDownloadCsv = async (filters) => {
    ...
    const filtered_alerts = await tagService.getFilteredZoneMetrics(
      filters_req
    );
    ...
    const formatDownloadData = {
      title: 'HISTORIAL DE ENTRADAS Y SALIDAS DE BINS A ZONAS',
      table: [documentTitles, ...documentData],
    };
    downloadFile(formatDownloadData, 'historial_bins_zonas');
  };
  ...
  const tableHeaders = [
    ...
    { id: 'zone', label: 'Zona' },
    { id: 'in_time', label: 'Hora de Ingreso' },
    { id: 'out_time', label: 'Hora de Egreso' },
    ...
    <div className={classes.container}>
      <div className={classes.tableTitleContainer}>
        ...
        <Typography className={classes.tableTitle} variant="h5">
          {'HISTORIAL DE ENTRADAS Y SALIDAS DE BINS A ZONAS'}
        </Typography>
      </div>
      <SearchAndButtons
